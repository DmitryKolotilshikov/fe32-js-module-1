...
// моя версия была такая, но она не работает(
...
function trimString(string, value1, value2) {
    return string.slice(value1, value2);
}
console.log(trimString('аргумент', 2, 1));
// (она в консоль так и не вывелась. не знаю почему)

// Task 4 💻
// Написать функцию getSumNumbers, которая будет принимать число и вычислять сумму цифр из которых состоит число.
// Для 2021 это будет 5. После того, как строку преобразвали в число - дальше как разбить и сложить я не знаю. Поэтому списала.
...
// отрицательными, найти сумму всех чисел между ними, включая их, и вернуть ее.Если два числа равны,
//     верните a или b.
//     getSum(1, 0) == 1   // 1 + 0 = 1
//     getSum(1, 2) == 3   // 1 + 2 = 3
//     getSum(0, 1) == 1   // 0 + 1 = 1
...
getSum2(2, 6)
//  в одной функции не смогла объединить эти три условия, поэтому раздельно
let getSum3 = (a, b) =>
{
let sum = 0;
for (let i = a; i > b; i--) {
...
let fooBoo = prompt('Напишите true или false');
let message = (fooBoo = 'true') ? 'foo' : 'boo';
alert(message);
